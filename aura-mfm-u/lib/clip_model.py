...
class ClipPLModel(nn.Module):
    ...
    def __init__(self, freeze=True, device='cpu', *args, **kwargs):
    ...
    def forward(self, img: np.array, labels: Optional[List] = None, device: Optional[str] = None):
    ...
    def get_text_embeddings(self, text: List[str], device: Optional[str] = None):
        ...
        max_token_length = 32
        text = [' '.join(t.split()[:max_token_length]) if len(t.split()) > max_token_length else t for t in text]
