...
def train_downstream(configs):
    ...
    model_identifier = f"{model_name}_{str_modality_initials}_ie_{imu_encoder_name}_w_{window_sec}"
    if model_suffix != "":
    ...
    if zero_shot:
        ...
        model = ClassificationModule(
            model=ZeroShotClassification(encoder=encoder, text_encoder=text_encoder, label_texts=label_texts)
        )
    else:
        # Initialize the training module for the classification model
        model = ClassificationModule(model=Head(encoder=encoder, size_embeddings=512, n_classes=datamodule.n_classes))
    # Checkpoint settings
...
if __name__ == "__main__":
    ...
    parser.add_argument("--path_configs", default="./configs/train_downstream/default.yaml")
