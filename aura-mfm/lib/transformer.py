...
class Transformer(pl.LightningModule):
    ...
    def forward(self, x, mask=None):
        ...
        # head
        if self.num_classes > 0 or not self.is_head:
...
def SS(**args):
    return Transformer(dim=192, nheads=6, nlayers=6, **args)
def S(**args):
    return Transformer(dim=384, nheads=6, nlayers=12, **args)
def B(**args):
    return Transformer(dim=768, nheads=12, nlayers=12, **args)
def L(**args):
    return Transformer(dim=1024, nheads=16, nlayers=24, **args)
