...
import torch
from dataset.ego4d.dataloader import Ego4dDataset, Ego4dDatasetSupervised, collate_wrapper
from dataset.ego4d.dataloader_unsupervised import Ego4dDatasetUnsupervised
from dataset.ego4d.utils.utils import load_csv, load_json, get_ego4d_metadata, get_ego4d_metadata_uid
import logging
random.seed(0)
...
class Split(object):
    def __init__(
        ...
        self.set = load_json(f"./splits_exo/{split}_{random_split}.json")
    ...
    def scale(self, video_uid_sample_rate: float):
        ...
        logging.info(f"Reducing to {n_videos_to_return} videos ...")
...
class Ego4dDataModule(pl.LightningDataModule):
    ...
    def __init__(self, batch_size, num_workers, pin_memory, drop_last, dataset_params, split_num, data_path):
        ...
        self.meta_video = get_ego4d_metadata_uid("video",data_path)
        self.collate_fn = lambda data: collate_wrapper(data, self.dataset_params["list_modalities"])
        # 他の初期化処理
        self.allow_zero_length_dataloader_with_multiple_devices = False  # または False

        # Get splits ready
        self.filter_video_uids_train = Split(random_split=split_num, split="training")
        self.filter_video_uids_validation = Split(random_split=split_num, split="validation")
        self.filter_video_uids_test = Split(random_split=split_num, split="test")
    ...
    def get_dataset(
        ...
        return Ego4dDataset(
            ...
            clean_narration_func=(
                self.dataset_params["clean_narration_func"]
                if self.dataset_params["clean_narration_func"]
                else lambda x: x
            ),
            filter_narration_func=(
                self.dataset_params["filter_narration_func"]
...
class UnsupEgo4dDataModule(pl.LightningDataModule):
    ...
    def get_dataset(
        ...
        return Ego4dDatasetUnsupervised(
            ...
            return_tuple=False,
            target_frames_in_window=self.dataset_params["target_fps"],
            filter_video_names=filter_video_uids_split.filter,
            window_sample_rate=window_sample_rate,
            max_n_windows_per_video=max_n_windows_per_video,
