...
class AttentionPooling(torch.nn.Module):
    def __init__(self, input_channels):
        ...
        self.weight = torch.nn.Conv1d(in_channels=input_channels, out_channels=1, kernel_size=1)
...
class TimeDistributedIMUEncoder(pl.LightningModule):
    ...
    def __init__(self, n_frames=10, n_channels=6, n_steps_per_frame=128, size_embeddings=512):
...
class PatchTransformer(pl.LightningModule):
    ...
    def __init__(
        ...
        self.imu_token_embed = torch.nn.Linear(imu_patch_dims, size_embeddings, bias=False)
    ...
    def forward(self, batch):
        ...
        x = batch.unfold(-1, self.imu_patch_size, self.imu_patch_size).permute(0, 2, 1, 3)
...
class MW2StackRNNPooling(pl.LightningModule):
    def __init__(self, input_dim=32, size_embeddings: int = 128):
        ...
        self.net = torch.nn.Sequential(
            ...
            torch.nn.GRU(batch_first=True, input_size=input_dim, hidden_size=size_embeddings),
        )
    ...
    def forward(self, batch):
        ...
        x = self.net[0](batch)
        # print(f"After GroupNorm 1 shape: {x.shape}")
        x = self.net[1](x)
        # print(f"After Block 1 shape: {x.shape}")
        x = self.net[2](x)
        # print(f"After Block 2 shape: {x.shape}")
        x = self.net[3](x)
        # print(f"After Block 3 shape: {x.shape}")
        x = self.net[4](x)
        # print(f"After GroupNorm 2 shape: {x.shape}")
        x = torch.permute(x, (0, 2, 1))
        out, hidden = self.net[5](x)
