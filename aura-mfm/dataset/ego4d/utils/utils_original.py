...
def get_ego4d_metadata(types: str = "clip"):
    ...
    Get ego4d metadata
    """
    return {clip[f"{types}_uid"]: clip for clip in load_json(PATH_EGO_META)[f"{types}s"]}
...
def index_narrations():
    narration_raw = load_json("/datasets01/ego4d_track2/v2/annotations/narration.json")
