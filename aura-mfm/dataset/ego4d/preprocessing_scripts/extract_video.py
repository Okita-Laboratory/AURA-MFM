...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Downframe video to given fps and resize frames")
    parser.add_argument(
        "-v",
        "--video_dir",
        help="Directory with video files",
        default="../ego-exo4d-sample/videos",
    )
    parser.add_argument(
        "-o",
        "--output_dir",
        help="Output dir for audio clips",
        default="../../checkpoint/full_videos/processed_videos",
    )
    parser.add_argument("-f", "--fps", help="Target fps", default=10)
    parser.add_argument("-s", "--size", help="Target frame size", default=224)
