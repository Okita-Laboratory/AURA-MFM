...
def load_imu(vrs_path, duration):
    ...
    accl_x = df["accel_x"].fillna(0).tolist()
    accl_y = df["accel_y"].fillna(0).tolist()
    accl_z = df["accel_z"].fillna(0).tolist()
    ...
    timestamps = (df["timestamp_ms"] - start_time).to_numpy()
...
def get_ego4d_metadata():
    return {clip["take_name"]: clip["duration_sec"] for clip in load_json("../takes.json")}


def process_imu_files(video_dir, output_dir):
    os.makedirs(output_dir, exist_ok=True)
    meta_video = get_ego4d_metadata()
    for filename in tqdm(glob.glob(f"{video_dir}/*/*.vrs")):
        name_clip = filename.split("/")[-2]
        duration = meta_video[name_clip]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="/data4/dataSpace/EGO_EXO4D/takes",
    )
    parser.add_argument(
        "-o",
        "--output_dir",
        ...
        default="../../checkpoint/full_videos/processed_imu",
    )
