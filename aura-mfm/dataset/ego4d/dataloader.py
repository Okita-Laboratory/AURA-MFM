...
random.seed(1234)
DATA_PATH = "./dataset/checkpoint/full_videos"
...
class Ego4dDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.narr = narr
        # load narration
        # logging.info("Loading narrations")
        narration_dict = index_narrations(self.data_path)

        self.window_idx = []
        for video_uid, narrations in tqdm(narration_dict.items()):
            if video_uid not in self.meta_video.keys():
                logging.info(f"Video {video_uid} not found in metadata")
                continue
            if not filter_video_uids(self.meta_video[video_uid]["take_name"]):
                # logging.info(f"Video {video_uid} filtered out")
                continue
            # if not self.check_modality_clip_uid(video_uid):
            #     continue
            video_duration = self.meta_video[video_uid]["duration_sec"] - 1
            ...
            for timestamp, text in narrations:
                if not filter_narration_func(text):
                    logging.info(f"Filtered narration: {text}")
                    continue
                if max_n_windows_per_video is not None and n_windows_per_video >= max_n_windows_per_video:
                    logging.info(f"Reached max windows for video {video_uid}")
                    continue
                if window_sample_rate != 1.0 and random.random() > window_sample_rate:
                    logging.info(f"Skipped window for video {video_uid}")
                    continue
                if video_duration < window_sec:
                    logging.info(f"Video {video_uid} is too short")
                    continue
                    

                # check if it's the timestamp is at the very beginning
                if timestamp <= window_sec/2.0:
                    w_s = 0.0
                    w_e = window_sec
                # check if it's the time stamp is at the very end
                elif timestamp + window_sec/2.0 >= video_duration:
                    w_s = video_duration - window_sec
                    w_e = video_duration
                # else get a window of data around the timestamps
                else:
                    w_s = timestamp - window_sec/2.0
                    w_e = timestamp + window_sec/2.0
                ...
                try:
                    ...
                    logging.info(f"Window size is not correct: {w_s} - {w_e} = {window_sec}")
                    continue
    ...
    def check_modality_clip_uid(self, video_uid):
        ...
        if self.imu and not has_imu:
            return False
        if self.audio and (
            not has_audio or not os.path.exists(os.path.join(self.data_path, f"processed_audios/{video_uid}.wav"))
        ):
            return False
        return True
    ...
    def __getitem__(self, idx):
        ...
        uid = self.meta_video[dict_out["video_uid"]]["take_name"]
        ...
        if self.audio:
            dict_out["audio"] = get_audio_frames(
                audio_fn=os.path.join(self.data_path, f"processed_audios/{uid}.wav"),
                video_start_sec=w_s,
                video_end_sec=w_e,
            )
            # TODO find a different solution for corrupted audio
            if dict_out["audio"]["signal"].shape[1] != int(self.window_sec * 2) * 16000:
                # logging.info("BAD AUDIO ...")
                dict_out["audio"]["signal"] = torch.zeros(1, int(self.window_sec * 2) * 16000)
        ...
        if self.imu:
            ...
            if dict_out["imu"] is None:
                logging.info("BAD IMU shouldn't be here")
                dict_out["imu"] = {"signal": torch.zeros(6, int(self.window_sec * 2) * 200)}
...
class Ego4dDatasetSupervised(torch.utils.data.Dataset):
    ...
    def check_modality_clip_uid(self, video_uid):
        ...
        if self.imu and not has_imu:
            return False
        if self.audio and (
            not has_audio or not os.path.exists(os.path.join(DATA_PATH, f"processed_audios/{video_uid}.wav"))
        ):
            return False
        return True
