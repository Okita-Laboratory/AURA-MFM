...
class CalcLoss(nn.Module):
    def __init__(self, modality, source='imu', lossfn=InfoNCE(symmetric_loss=True, learn_temperature=True)):
        super().__init__()
        self.loss_fn = lossfn
        self.target = copy.copy(modality)
        self.target.remove(source)
        self.source = source
    def forward(self, out):
        loss_output = 0.0
        query = out[self.source]
        for target_modality in self.target:
            key = out[target_modality]
            s2t_loss = self.loss_fn(query=query, positive_key=key)
